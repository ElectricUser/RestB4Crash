...
async def calculateUserTodaysWork(users):
    removed_tasks = []
    for user in users:
        task_done = []
        task_todo = []
...
async def calculateUserTommorrowWork(users):
    usersTaskInfo = {}
    for count, user in enumerate(users):
        ...
        usersTaskInfo[user] = ({
            ...
            "difficulty_todo_tommorrow": 15 - difficulty_todo - getConversion(n_stress,
                                                                              n_pauses) if 15 - difficulty_todo - getConversion(
                n_stress, n_pauses) > 0 else 0
        })
...
def sortUsers(users, usersTaskInfo, debug=False):
    ...
    users = sorted(users, key=lambda user: usersTaskInfo.get(user)[
                                               'hours_todo_tommorrow'] + usersTaskInfo.get(user)[
                                               'difficulty_todo_tommorrow'] / 2, reverse=True)
...
async def storeHistoricTask(tasks):
    uri = "mongodb+srv://grupo3meia:Grupo3isbest@grupo3meia.twv654h.mongodb.net/?retryWrites=true&w=majority"
...
async def storeAssignedTask(tasks):
    uri = "mongodb+srv://grupo3meia:Grupo3isbest@grupo3meia.twv654h.mongodb.net/?retryWrites=true&w=majority"
    ...
    for task in tasks:
        collection.delete_one({"title": task['title']})
...
async def distributeTask():
    tasks, assignedTask, users = await getData()

    # Gets the users
    # Remove this task and Store in the DB as "Historic Task"
    users, task2Remove = await calculateUserTodaysWork(users)

    # Gets User availably for tommorrow
    usersTaskInfo = await calculateUserTommorrowWork(users)
    ...
    await storeAssignedTask(formatAddedTask(task_added))
